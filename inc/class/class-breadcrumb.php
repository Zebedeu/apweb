...
function apweb_breadcrumbs( $args = array() ) {
	...
	$defaults = array(
		'separator' => '&raquo;',
		'before' => '<span class="breadcrumb-title">' . __( 'You are here:', 'apweb') . '</span>',
		'after' => false,
		'front_page' => true,
		'show_home' => __( 'Home', 'apweb'),
		'echo' => false, 
	...
	elseif ( is_archive() ) {
		...
		elseif ( is_time() ) {
			...
			if ( get_query_var( 'minute' ) && get_query_var( 'hour' ) )
				$trail['trail_end'] = get_the_time( __( 'g:i a', 'apweb') );

			elseif ( get_query_var( 'minute' ) )
				$trail['trail_end'] = sprintf( __( 'Minute %1$s', 'apweb'), get_the_time( __( 'i', 'apweb') ) );

			elseif ( get_query_var( 'hour' ) )
				$trail['trail_end'] = get_the_time( __( 'g a', 'apweb') );
		}
		...
		elseif ( is_date() ) {
			...
			if ( is_day() ) {
				$trail[] = '<a href="' . get_year_link( get_the_time( 'Y' ) ) . '" title="' . get_the_time( esc_attr__( 'Y', 'apweb') ) . '">' . get_the_time( __( 'Y', 'apweb') ) . '</a>';
				$trail[] = '<a href="' . get_month_link( get_the_time( 'Y' ), get_the_time( 'm' ) ) . '" title="' . get_the_time( esc_attr__( 'F', 'apweb') ) . '">' . get_the_time( __( 'F', 'apweb') ) . '</a>';
				$trail['trail_end'] = get_the_time( __( 'j', 'apweb') );
			}

			elseif ( get_query_var( 'w' ) ) {
				$trail[] = '<a href="' . get_year_link( get_the_time( 'Y' ) ) . '" title="' . get_the_time( esc_attr__( 'Y', 'apweb') ) . '">' . get_the_time( __( 'Y', 'apweb') ) . '</a>';
				$trail['trail_end'] = sprintf( __( 'Week %1$s', 'apweb'), get_the_time( esc_attr__( 'W', 'apweb') ) );
			}

			elseif ( is_month() ) {
				$trail[] = '<a href="' . get_year_link( get_the_time( 'Y' ) ) . '" title="' . get_the_time( esc_attr__( 'Y', 'apweb') ) . '">' . get_the_time( __( 'Y', 'apweb') ) . '</a>';
				$trail['trail_end'] = get_the_time( __( 'F', 'apweb') );
			}

			elseif ( is_year() ) {
				$trail['trail_end'] = get_the_time( __( 'Y', 'apweb') );
			}
		}
	}
	...
	elseif ( is_search() )
		$trail['trail_end'] = sprintf( __( 'Search results for &quot;%1$s&quot;', 'apweb'), esc_attr( get_search_query() ) );

	/* If viewing a 404 error page. */
	elseif ( is_404() )
		$trail['trail_end'] = __( '404 Not Found', 'apweb');
