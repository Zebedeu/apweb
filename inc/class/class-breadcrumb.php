...
if(!class_exists('apweb_breadcrumb')) {
    ...
    class Apweb_breadcrumb
    {
        ...
        function __construct($options = "")
        {
            ...
            $output = $this->apweb_simple_breadcrumb_case($post);
         ...
         function apweb_simple_breadcrumb_case($der_post)
        {
            global $post;
            ...
            if (is_page()) {
                if ($der_post->post_parent) {
                    ...
                    $this->apweb_simple_breadcrumb_case($my_query);
                    $link = '<a href="';
		...
		if(is_tax()){
            ...
            if (is_404()) {
                return _e("404 - Page not Found", 'k7themes');
            }

            if (is_search()) {
                return _e("Search", 'k7themes');
            }
        ...
        /*-----------------------------------------------------------------------------------*/
        /* apweb_breadcrumbs() - Custom breadcrumb generator function  */
        /*
        /* Params:
        /*
        /* Arguments Array:
        ...
        static function apweb_breadcrumbs($args = array())
        {
            ...
            $defaults = array(
                'separator' => '&raquo;',
                'before' => '<span class="breadcrumb-title">' . __('You are here:', 'k7themes') . '</span>',
                'after' => false,
                'front_page' => true,
                'show_home' => __('Home', 'k7themes'),
                'echo' => false,
            ...
            $args = apply_filters('apweb_breadcrumbs_args', $args);
            ...
            elseif (is_home()) {
                ...
                $trail = array_merge($trail, apweb_breadcrumbs_get_parents($home_page->post_parent, ''));
            ...
            elseif (is_singular()) {
                ...
                if ('page' !== $post_type && 'post' !== $post_type) {
                    ...
                    if (!empty($path))
                        $trail = array_merge($trail, apweb_breadcrumbs_get_parents('', $path));
                ...
                if (empty($path) && 0 !== $parent || 'attachment' == $post_type)
                    $trail = array_merge($trail, apweb_breadcrumbs_get_parents($parent, ''));
                ...
                if ('post' == $post_type && $show_posts_page == true && 'page' == get_option('show_on_front')) {
                    ...
                    if ($posts_page != '' && is_numeric($posts_page)) {
                        $trail = array_merge($trail, apweb_breadcrumbs_get_parents($posts_page, ''));
                    }
                }
            ...
            elseif (is_archive()) {
                ...
                if (is_tax() || is_category() || is_tag()) {
                    ...
                    if (is_taxonomy_hierarchical($term->taxonomy) && $term->parent)
                        $trail = array_merge($trail, apweb_breadcrumbs_get_term_parents($term->parent, $term->taxonomy));
                ...
                elseif (is_time()) {
                    ...
                    if (get_query_var('minute') && get_query_var('hour'))
                        $trail['trail_end'] = get_the_time(__('g:i a', 'k7themes'));

                    elseif (get_query_var('minute'))
                        $trail['trail_end'] = sprintf(__('Minute %1$s', 'k7themes'), get_the_time(__('i', 'k7themes')));

                    elseif (get_query_var('hour'))
                        $trail['trail_end'] = get_the_time(__('g a', 'k7themes'));
                } /* If viewing a date-based archive. */
                elseif (is_date()) {

                    /* If $front has been set, check for parent pages. */
                    if ($wp_rewrite->front)
                        $trail = array_merge($trail, apweb_breadcrumbs_get_parents('', $wp_rewrite->front));

                    if (is_day()) {
                        $trail[] = '<a href="' . get_year_link(get_the_time('Y')) . '" title="' . get_the_time(esc_attr__('Y', 'k7themes')) . '">' . get_the_time(__('Y', 'k7themes')) . '</a>';
                        $trail[] = '<a href="' . get_month_link(get_the_time('Y'), get_the_time('m')) . '" title="' . get_the_time(esc_attr__('F', 'k7themes')) . '">' . get_the_time(__('F', 'k7themes')) . '</a>';
                        $trail['trail_end'] = get_the_time(__('j', 'k7themes'));
                    } elseif (get_query_var('w')) {
                        $trail[] = '<a href="' . get_year_link(get_the_time('Y')) . '" title="' . get_the_time(esc_attr__('Y', 'k7themes')) . '">' . get_the_time(__('Y', 'k7themes')) . '</a>';
                        $trail['trail_end'] = sprintf(__('Week %1$s', 'k7themes'), get_the_time(esc_attr__('W', 'k7themes')));
                    } elseif (is_month()) {
                        $trail[] = '<a href="' . get_year_link(get_the_time('Y')) . '" title="' . get_the_time(esc_attr__('Y', 'k7themes')) . '">' . get_the_time(__('Y', 'k7themes')) . '</a>';
                        $trail['trail_end'] = get_the_time(__('F', 'k7themes'));
                    } elseif (is_year()) {
                        $trail['trail_end'] = get_the_time(__('Y', 'k7themes'));
                    }
                }
            } /* If viewing search results. */
            elseif (is_search())
                $trail['trail_end'] = sprintf(__('Search results for &quot;%1$s&quot;', 'k7themes'), esc_attr(get_search_query()));

            /* If viewing a 404 error page. */
            elseif (is_404())
                $trail['trail_end'] = __('404 Not Found', 'k7themes');

            /* Allow child themes/plugins to filter the trail array. */
            $trail = apply_filters('apweb_breadcrumbs_trail', $trail, $args);
            ...
            $breadcrumb = apply_filters('apweb_breadcrumbs', $breadcrumb);
        ...
        } // End apweb_breadcrumbs()


        function apweb_breadcrumbs_get_parents($post_id = '', $path = '')
        {
        ...
        } // End apweb_breadcrumbs_get_parents()


        function apweb_breadcrumbs_get_term_parents($parent_id = '', $taxonomy = '')
        {
        ...
        } // End apweb_breadcrumbs_get_term_parents()
    }
}